...
WINVER="$(timeout --kill-after=15s 10s $WINEATOMIC winecfg /v | cat)" && echo "saved WINVER ${WINVER}" && ($WINEATOMIC winecfg /v "${WINVER}" | cat) && (timeout --kill-after=15s 10s $WINEATOMIC winecfg /v | cat)

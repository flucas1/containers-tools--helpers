...
MICRONUCLEUSURL="https://web.archive.org/web/20241214221237id_/https://azduino.com/bin/micronucleus/${MICRONUCLEUSFILENAME}"
MICRONUCLEUSLOCALCACHEDIRECTORY="${ARG_CACHEPATH}"
if [ -z "${MICRONUCLEUSLOCALCACHEDIRECTORY}" ] ; then
  MICRONUCLEUSLOCALCACHEDIRECTORY="/tmp/micronucleus"
fi
mkdir -p "${MICRONUCLEUSLOCALCACHEDIRECTORY}"
MICRONUCLEUSLOCALCACHEFILENAME="${MICRONUCLEUSLOCALCACHEDIRECTORY}/${MICRONUCLEUSFILENAME}"
if [ ! -f "${MICRONUCLEUSLOCALCACHEFILENAME}" ] ; then
  ${HELPERSPATH}/wget-with-retries.sh "${MICRONUCLEUSURL}" "${MICRONUCLEUSLOCALCACHEFILENAME}"
fi
[ -f "${MICRONUCLEUSLOCALCACHEFILENAME}" ]
MICRONUCLEUSFINALTARGET="${PATH_STAGING}/packages/${MICRONUCLEUSFILENAME}"
cp "${MICRONUCLEUSLOCALCACHEFILENAME}" "${MICRONUCLEUSFINALTARGET}"
...
#${HELPERSPATH}/wget-with-retries.sh "${MICRONUCLEUSURL}" "${MICRONUCLEUSLOCAL}"
#rm -f $JSONTEMP
